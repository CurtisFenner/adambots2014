...
?>
<?php
function make_result($searchterms) {
	$t_init = tim();

	$section = '<section style="font-size:16px;"><num><h2><a href="';
	...
	$str = preg_replace('/<\/b>([^<>]{40}\S*)[\s\S]*?(\S*[^<>]{50})<b>/','</B>$1 &hellip; $2<B>',$str,11);
	$str = str_replace("\n","<br>",$str);
	$str = str_replace("FIRST","<em>FIRST</em>",$str);
	$str = str_replace("FRC","<em>FRC</em>",$str);
	$str = str_replace("OCCRA","<em>OCCRA</em>",$str);
	$section = $section . $str . "<br><br><p class=dim style=font-style:italic;>Relevance: <b><rel></b></p></section><hr>";
	return array($count * (500 / (strlen($counter) + 20) + 1),$section, $t_init,$t_regex,$t_bold,$t_dots,tim() - $t_end);
}
?>